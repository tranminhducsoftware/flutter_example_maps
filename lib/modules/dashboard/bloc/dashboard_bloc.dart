...
import 'package:maps/modules/dashboard/bloc/dashboard_events.dart';
import 'package:maps/modules/dashboard/bloc/dashboard_states.dart';

class DashboardBloc {
  var state = DashboardState(70);

  // tạo 2 controller
  // 1 cái quản lý event, đảm nhận nhiệm vụ nhận event từ UI
  final eventController = StreamController<DashboardEvents>();

  // 1 cái quản lý state, đảm nhận nhiệm vụ truyền state đến UI
  final stateController = StreamController<DashboardState>();


  DashboardBloc() {
    // lắng nghe khi eventController push event mới
    eventController.stream.listen((DashboardEvents event) {
